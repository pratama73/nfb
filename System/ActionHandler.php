...
class ActionHandler
{
    ...
    private function addFriend($url)
    {
        ...
        } elseif (strpos($src, "removefriend.php")!==false) {
    ...
    private function avoidBruteLogin()
    {
        if (file_exists($this->loginCountFile)) {
        	$cond = (int)file_get_contents($this->loginCountFile)<10;
        	if (!$cond) {
        		print $a = "Malformed login ". $this->email. "\n\n".date("Y-m-d H:i:s");
        		$this->report($a);
        		die;
        	}
            return $cond;
